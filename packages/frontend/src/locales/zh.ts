...
export const messages: LocaleMessages = {
    ...
    configPanel: {
        ...
        agentPromptTemplate2:  '根据用户的输入："${promptResults1}"，来优化 json中的promptBlock中的提示词，这个 json 是一个问卷，用户会输入 inputB1-N，然后点击提交后，顺序调用 promptBlock1-N中的字符串给到 ai，并依次把内容返回给用户。需要你优化promptBlock中的提示词：1，确保promptBlock1中的提示词引用了正确的用户输入${inputBn}，确保后续的提示词正确地引用了它前面的上下文${promptBlockn}，n为1-n的整数，代表了控件的编号，只输出JSON文件，不要输出任何其他内容',
        agentTitle: 'Agent生成器',

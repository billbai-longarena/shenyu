...
export class SpeedTestButtonWebSocket {
    private static instance: SpeedTestButtonWebSocket;
    private connections: Map<string, WebSocket> = new Map();
    private listeners: Map<string, EventListeners> = new Map();

    private constructor() { }

    static getInstance(): SpeedTestButtonWebSocket {
        if (!SpeedTestButtonWebSocket.instance) {
            SpeedTestButtonWebSocket.instance = new SpeedTestButtonWebSocket();
        }
        return SpeedTestButtonWebSocket.instance;
    }

    async getConnection(blockIndex: number, model: ModelType): Promise<WebSocket> {
        const timestamp = Date.now();
        const key = `speed_test_${model}_${timestamp}`;
        ...
        for (const [key, ws] of this.connections.entries()) {
            try {
                ...
                if (currentTime - timestamp > 30000) {
                    ...
                    console.log('[SpeedTest Button WebSocket] 清理连接:', {
                        key,
                ...
                console.error('[SpeedTest Button WebSocket] 清理连接失败:', {
                    key,
                    error,
                    timestamp: new Date().toISOString()
                });

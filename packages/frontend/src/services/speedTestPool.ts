...
export class SpeedTestPool {
    private static instance: SpeedTestPool;
    private connections: Map<string, WebSocket> = new Map();
    private listeners: Map<string, EventListeners> = new Map();

    private constructor() { }

    static getInstance(): SpeedTestPool {
        if (!SpeedTestPool.instance) {
            SpeedTestPool.instance = new SpeedTestPool();
        }
        return SpeedTestPool.instance;
    }

    async getConnection(model: ModelType): Promise<WebSocket> {
        ...
        for (const [key, ws] of this.connections.entries()) {
            try {
                ...
                if (currentTime - timestamp > 30000) {
                    ...
                    console.log('[SpeedTest Pool] 清理连接:', {
                        key,
                ...
                console.error('[SpeedTest Pool] 清理连接失败:', {
                    key,
                    error,
                    timestamp: new Date().toISOString()
                });

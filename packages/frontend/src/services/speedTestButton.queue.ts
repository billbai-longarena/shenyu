...
export class SpeedTestButtonQueue {
    private queue: QueueItem[] = [];
    private activeRequests = new Map<string, Promise<void>>();
    private maxConcurrent = 4; // 固定并发数为4
    private lastErrorTime: number | null = null;
    private static instance: SpeedTestButtonQueue;

    private constructor() { }

    static getInstance(): SpeedTestButtonQueue {
        if (!SpeedTestButtonQueue.instance) {
            SpeedTestButtonQueue.instance = new SpeedTestButtonQueue();
        }
        return SpeedTestButtonQueue.instance;
    }

    async enqueue(model: ModelType, onChunk: (chunk: string) => void): Promise<void> {
        console.log(`[SpeedTest Button Queue] Enqueueing model test:`, {
            model,
        ...
        const requestPromise = (async () => {
            try {
                ...
                if (this.lastErrorTime) {
                    ...
                    console.log(`[SpeedTest Button Queue] Waiting ${waitTime}ms before retry for model ${request.model}`);
                ...
                // 导入 SpeedTestButtonWebSocket
                const { SpeedTestButtonWebSocket } = await import('./speedTestButton.websocket.js');
                const pool = SpeedTestButtonWebSocket.getInstance();
                const ws = await pool.getConnection(0, request.model); // 测速模式使用block 0
                ...
                console.error(`[SpeedTest Button Queue] Error processing model ${request.model}:`, error);
    ...
    private async handleError(request: QueueItem, error: any): Promise<void> {
        ...
        console.log(`[SpeedTest Button Queue] Handling error for model ${request.model}:`, {
            error: error.message,
        ...
        if (request.retryCount < maxRetries && isRetryable) {
            ...
            console.log(`[SpeedTest Button Queue] Will retry model ${request.model} (attempt ${request.retryCount})`);
        } else {
            console.log(`[SpeedTest Button Queue] Model ${request.model} test failed permanently`);
